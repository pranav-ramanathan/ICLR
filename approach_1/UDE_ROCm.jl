...
UDE_ROCm.jl
===========
ROCm-first adaptive UDE-lite runner for N3L.

Design notes:
- Intentionally no GPU+CPU hybrid orchestration.
- Starts from existing ROCm solver (main_rocm.jl) and adds an adaptive controller
  over (α, γ, T) across rounds.
- This is the simple starting point; richer learned corrections can be layered later.
=#
...
function adapt_params!(c::UDEConfig, round::Int)
    # Alternate exploration direction to escape local minima without brute sweeps.
    if isodd(round)
...
function parse_ude_args(args)
    s = ArgParseSettings(description="Adaptive UDE-lite ROCm runner for N3L")
...
function main()
    ...
    for round in 1:ude.rounds
        ...
        ok, grid, elapsed, stats, _ = solve_rocm(
            cfg,
            line_offsets,
            line_points,
            point_col_scale,
        ...
        if ok
            verbose && @printf("UDE-ROCm SUCCESS at round %d (%.2fs)\n", round, elapsed)
            return 0
        end
    ...
    verbose && println("UDE-ROCm: no solution found within adaptive budget.")
    return 1
end
