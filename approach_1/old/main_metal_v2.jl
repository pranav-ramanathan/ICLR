#!/usr/bin/env julia
#=
N3L Pure Gradient Flow — Metal GPU Accelerated (Line-Based RK4)
===============================================================
This version replaces explicit collinear-triple enumeration with a line
representation that is mathematically equivalent for both:
  - Gradient contributions
  - Violation counting

Key improvements vs `main_metal.jl`:
  - O(n^4) line precompute instead of O(n^6) triple precompute
  - Line-based kernel with exact triple-equivalent gradient
  - Better coefficient selection (`--coeff-mode`) for larger n
=#
...
function choose_coefficients(
    ...
    elseif mode_l == "density"
        ...
        error("Invalid --coeff-mode '$mode'. Expected one of: legacy, density")
    end
...
function parse_cli_args(args)
    s = ArgParseSettings(
        description = "N3L Gradient Flow — Metal GPU (Line-Based Custom Kernel)",
        version = "3.0.0",
        add_version = true
    )
    ...
    @add_arg_table! s begin
        ...
        "--gamma"
            ...
            arg_type = Float64
        "--coeff-mode"
            help = "Coefficient strategy when not overridden: legacy | density"
            arg_type = String
            default = "density"
        "--seed"
            help = "Random seed"
...
function solve_metal(
    ...
    verbose && println("="^60)
    verbose && println("N3L Gradient Flow — Metal GPU (Line-Based RK4 Kernel)")
    verbose && println("="^60)
    ...
    try
        ...
        kern(warmup_state, d_line_offsets, d_line_points,
    ...
    for batch_idx in 1:total_batches
        ...
        kern(d_state, d_line_offsets, d_line_points,
...
function save_solution(n, grid, traj_id, R, T, seed, α, γ, outdir)
    ...
    dir = "$(outdir)/$(n)"
    mkpath(dir)
    filename = "$(dir)/sol_metal_v2_$(timestamp)_traj$(traj_id).txt"
    ...
    open(filename, "w") do io
        ...
        println(io, "# method=Metal GPU Line-Based Custom RK4 Kernel")
...
function main()
    ...
    if success
        ...
        println("julia --project=. $(PROGRAM_FILE) $(n) --R $(R) --T $(T) --dt $(dt) --alpha $(cfg.α) --gamma $(cfg.γ) --seed $(used_seed) --batch-size $(batch_size) --max-batches $(max_batches) --outdir $(outdir)")
    end
