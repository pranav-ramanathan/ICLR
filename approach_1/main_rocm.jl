...
function parse_cli_args(args)
    s = ArgParseSettings(
        description = "N3L Gradient Flow — ROCm AMD GPU (Line-Based Custom Kernel v1)",
        version = "4.0.0",
        add_version = true
    )
...
function solve_rocm(
    ...
    verbose && println("="^60)
    verbose && println("N3L Gradient Flow — ROCm AMD GPU (Line-Based RK4 Kernel v1)")
    verbose && println("="^60)
...
function save_solution(n, grid, traj_id, R, T, seed, α, γ, outdir; col_normalization::String="mean-incidence")
    ...
    dir = "$(outdir)/$(n)"
    mkpath(dir)
    filename = "$(dir)/sol_rocm_v1_$(timestamp)_traj$(traj_id).txt"
    ...
    open(filename, "w") do io
        ...
        println(io, "# method=ROCm AMDGPU Line-Based Custom RK4 Kernel (v1)")
...
function main()
    ...
    success, grid, elapsed, stats, used_seed = solve_rocm(
        cfg,
        line_offsets_cpu,
        line_points_cpu,
        point_col_scale_cpu,
...
if abspath(PROGRAM_FILE) == @__FILE__
    exit(run_with_terminal_log("rocm_v1", ARGS) do
        main()
    end)
end
